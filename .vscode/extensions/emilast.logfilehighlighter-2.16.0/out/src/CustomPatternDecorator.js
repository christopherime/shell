    ...
    updateConfiguration() {
        ...
        for (const item of configPatterns) {
            ...
            if ((item.foreground !== undefined
                ...
                var pattern = new CustomPattern(item.pattern, item.foreground, item.background, item.fontWeight, item.fontStyle, item.border, item.borderRadius, item.borderSpacing, item.letterSpacing, item.overviewColor, vscode.OverviewRulerLane[item.overviewRulerLane], item.textDecoration);
