...
export async function resolve(specifier, context, defaultResolve) {
  ...
  if (tracer && tracer._esm) {
    ...
    if (!specifier.startsWith('file:') && !specifier.startsWith('data:') && !specifier.startsWith('.')) {
            ...
            try {
              ...
              if (nestedError.code === 'ERR_MODULE_NOT_FOUND') {
                try {
                  ...
                  if (nestedNestedError.code === 'ERR_MODULE_NOT_FOUND' && esm.tsNodeResolve) {
                    context.parentURL = parentURL;
                    const { url } = esm.tsNodeResolve.defaultResolve(specifier, context, defaultResolve);
                    return resolve(url, context, defaultResolve);
                  } else {
                    throw nestedNestedError;
                  }
                }
              } else {
                throw nestedError;
      ...
      specifier = `${specifier}?session=${global.$_$session}${Date.now()}`;
