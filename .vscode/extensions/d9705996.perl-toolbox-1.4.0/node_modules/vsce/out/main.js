...
function main(task) {
    ...
    if (tty_1.isatty(1)) {
        npm_1.getLatestVersion(pkg.name, token)
            ...
            .catch(err => !util_1.isCancelledError(err) && util_1.log.error(err));
    }
    task
        .catch(fatal)
        .then(() => {
...
module.exports = function (argv) {
    ...
    program
        ...
        .option('--noVerify')
        .action((version, { pat, packagePath, baseContentUrl, baseImagesUrl, yarn, noVerify }) => main(publish_1.publish({ pat, version, packagePath, baseContentUrl, baseImagesUrl, useYarn: yarn, noVerify })));
    program
